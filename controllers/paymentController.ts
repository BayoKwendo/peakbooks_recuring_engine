...
*
  ...
  */
   ...
   */
	...
	//Delete banks amount
	deleteBankAmount: async ({ request, response }: { request: any, response: any }) => {
		try {
			const body = await request.body();
			...
			if (values.open_type == 'Petty Cash' || values.open_type == 'Undeposited Funds') {
				// values.amount_received = 0;
				console.log("Bayo", values.account_type);

				const mlet = await paymentService.deleteBankCash({
					amount_received: values.amount_received,
					id: values.id,
					account_type: values.account_type,
				});
